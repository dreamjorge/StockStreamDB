        ...
        try:
            ...
            if data.empty:
                raise YFPricesMissingError(
                    f"No prices found for {symbol} in the requested period."
                )
    ...
    def insert_bulk_data(self, symbol, data, source):
        ...
        if source == "YahooFinance" and not data.empty:
            ...
            self.cursor.executemany(
                """
                INSERT INTO stock_prices (symbol, timestamp, open, high, low,
                close, volume)
                VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    try:
        while True:
            ...
            if data is not None and not data.empty:
                ...
                logging.info(
                    f"Inserted {len(data)} records for {symbol} using {fetcher_type}"
                )
            else:
