...
@pytest.fixture
def mock_stock():
    return Stock(ticker="AAPL", name="Apple Inc.", industry="Technology", sector="Consumer Electronics", close=150.0, date=datetime(2024, 1, 1))
...
def test_update_stock(stock_repository, mock_stock):
    updated_stock = Stock(ticker="AAPL", name="Apple Inc.", industry="Technology", sector="Consumer Electronics", close=155.0, date=datetime(2024, 1, 1))
...
def test_get_stock_data(stock_repository, mock_stock):
    ...
    result = stock_repository.get_stock_data("AAPL", datetime(2022, 1, 1), datetime(2023, 1, 1), Granularity.DAILY)
    assert result == stock_data
    stock_repository.get_stock_data.assert_called_once_with("AAPL", datetime(2022, 1, 1), datetime(2023, 1, 1), Granularity.DAILY)
...
def test_get_stock_data_with_exception(stock_repository):
    ...
    with pytest.raises(Exception, match="Database error"):
        stock_repository.get_stock_data("AAPL", datetime(2022, 1, 1), datetime(2023, 1, 1), Granularity.DAILY)
