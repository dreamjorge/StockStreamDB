...
def test_get_stock_data(stock_repository, mock_session):
    ...
    df = stock_repository.get_stock_data('AAPL', start_date, end_date, Granularity.DAILY)

    # Assert the DataFrame has the correct data
    expected_df = pd.DataFrame({
        'date': pd.to_datetime([datetime(2024, 9, 26), datetime(2024, 9, 27), datetime(2024, 9, 28)]),
