import pytest
from src.interfaces.common.enums import (
    Granularity,
...
def test_resample_rule_invalid_granularity():
    # Test invalid granularity by passing a value that doesn't exist in the
    # Granularity enum
    invalid_granularity = "invalid"

    # Simulate passing the invalid granularity and ensure a ValueError is raised
    with pytest.raises(
        ValueError, match=f"'{invalid_granularity}' is not a valid Granularity"
    ):
