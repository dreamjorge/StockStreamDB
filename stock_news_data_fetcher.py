    ...
    def insert_news(self, symbol, news_list):
        ...
        if news_list:
            ...
            self.cursor.executemany(
                """
                INSERT OR IGNORE INTO stock_news (symbol, headline,
                source, datetime, summary, url)
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                rows,
            )
        ...
        if result and result[0]:
            ...
            if isinstance(latest_datetime, int):
                ...
                try:
                    ...
                    logging.error(
                        f"Invalid datetime format in database: {latest_datetime}"
                    )
                    return None
        else:
            return None
    ...
    def print_news_data(self):
        ...
        try:
            ...
            if rows:
                ...
                for row in rows:
                    date_str = datetime.fromtimestamp(row[4]).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    print(
...
def main():
    ...
    parser.add_argument(
        "--print-db",
        action="store_true",
        help="Print the news data stored in the database",
    )
    args = parser.parse_args()

    # Set up Finnhub API key and symbol
    api_key = os.getenv(
        "FINNHUB_API_KEY", "crib06hr01qqt33rgoh0crib06hr01qqt33rgohg"
    )  # Replace with your actual API key
    symbol = os.getenv(
        "STOCK_SYMBOL", "AAPL"
    )  # Default to Apple news, but can fetch for others
    ...
    try:
        ...
        while current_date < to_date:
            ...
            news_list = news_fetcher.fetch_news(
                symbol, from_date=batch_from_date, to_date=batch_to_date
            )
        ...
        logging.info(
            f"Total new articles added to the database: {total_articles_added}"
        )
        logging.info(
            f"Total articles in the database: {db_manager.get_total_articles()}"
        )
