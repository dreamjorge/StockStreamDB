...
import { normalize as normalize$2, resolve } from 'path';
...
const mapObject$1 = getDefaultExportFromCjs(mapObjExports);
...
const has$2 = (array, key) => array.some(element => {
...
const cache$3 = new QuickLRU({maxSize: 100_000});
...
const camelCaseConvert = (input, options) => {
	...
	const makeMapper = parentPath => (key, value) => {
		if (deep && isObject$1(value)) {
			...
			if (!stopPathsSet.has(path)) {
				value = mapObject$1(value, makeMapper(path));
			}
		}
		if (!(exclude && has$2(exclude, key))) {
			const cacheKey = pascalCase ? `${key}_` : key;
			if (cache$3.has(cacheKey)) {
				key = cache$3.get(cacheKey);
			} else {
				const returnValue = camelCase(key, {pascalCase, locale: false});
				if (key.length < 100) {
					cache$3.set(cacheKey, returnValue);
				}
				key = returnValue;
			}
		}
	...
	return mapObject$1(input, makeMapper(undefined));
};
...
var hardRejection$2 = {exports: {}};
const util$4 = require$$0;
let installed = false;
const hardRejection = (log = console.error) => {
	...
	process.on('unhandledRejection', error => {
		if (!(error instanceof Error)) {
			error = new Error(`Promise rejected with value: ${util$4.inspect(error)}`);
		}
		log(error.stack);
		process.exit(1);
	});
...
const hardRejection$1 = getDefaultExportFromCjs(hardRejectionExports);
const debug$1 = (
...
const hasACAbortSignal = typeof AC$1.AbortSignal === 'function';
const AS$1 = hasAbortSignal
  ? AbortSignal
  : hasACAbortSignal
...
const deprecatedOption = (opt, instead) => {
  ...
  if (shouldWarn$1(code)) {
    warn(code, `${opt} option`, `options.${instead}`, LRUCache$2);
  }
};
const deprecatedMethod = (method, instead) => {
  const code = `LRU_CACHE_METHOD_${method}`;
  if (shouldWarn$1(code)) {
    const { prototype } = LRUCache$2;
...
let Stack$1 = class Stack {
  constructor(max) {
    ...
    const UintArray = getUintArray$1(max);
...
let LRUCache$2 = class LRUCache {
  ...
  initializeTTLTracking() {
    ...
    this.updateItemAge = index => {
      this.starts[index] = this.ttls[index] !== 0 ? perf$1.now() : 0;
    };
...
const maybeJoin$1 = (...args) => args.every(arg => arg) ? args.join('') : '';
const maybeEncode$1 = (arg) => arg ? encodeURIComponent(arg) : '';
const formatHashFragment$1 = (f) => f.toLowerCase().replace(/^\W+|\/|\W+$/g, '').replace(/\W+/g, '-');
const defaults$1 = {
  sshtemplate: ({ domain, user, project, committish }) =>
    `git@${domain}:${user}/${project}.git${maybeJoin$1('#', committish)}`,
  sshurltemplate: ({ domain, user, project, committish }) =>
    `git+ssh://git@${domain}/${user}/${project}.git${maybeJoin$1('#', committish)}`,
  edittemplate: ({ domain, user, project, committish, editpath, path }) =>
    `https://${domain}/${user}/${project}${maybeJoin$1('/', editpath, '/', maybeEncode$1(committish || 'HEAD'), '/', path)}`,
  browsetemplate: ({ domain, user, project, committish, treepath }) =>
    `https://${domain}/${user}/${project}${maybeJoin$1('/', treepath, '/', maybeEncode$1(committish))}`,
  browsetreetemplate: ({ domain, user, project, committish, treepath, path, fragment, hashformat }) =>
    `https://${domain}/${user}/${project}/${treepath}/${maybeEncode$1(committish || 'HEAD')}/${path}${maybeJoin$1('#', hashformat(fragment || ''))}`,
  browseblobtemplate: ({ domain, user, project, committish, blobpath, path, fragment, hashformat }) =>
    `https://${domain}/${user}/${project}/${blobpath}/${maybeEncode$1(committish || 'HEAD')}/${path}${maybeJoin$1('#', hashformat(fragment || ''))}`,
  docstemplate: ({ domain, user, project, treepath, committish }) =>
    `https://${domain}/${user}/${project}${maybeJoin$1('/', treepath, '/', maybeEncode$1(committish))}#readme`,
  httpstemplate: ({ auth, domain, user, project, committish }) =>
    `git+https://${maybeJoin$1(auth, '@')}${domain}/${user}/${project}.git${maybeJoin$1('#', committish)}`,
  filetemplate: ({ domain, user, project, committish, path }) =>
    `https://${domain}/${user}/${project}/raw/${maybeEncode$1(committish || 'HEAD')}/${path}`,
  shortcuttemplate: ({ type, user, project, committish }) =>
    `${type}:${user}/${project}${maybeJoin$1('#', committish)}`,
  pathtemplate: ({ user, project, committish }) =>
    `${user}/${project}${maybeJoin$1('#', committish)}`,
  bugstemplate: ({ domain, user, project }) =>
    `https://${domain}/${user}/${project}/issues`,
  hashformat: formatHashFragment$1,
};
const hosts$3 = {};
hosts$3.github = {
  ...
  filetemplate: ({ auth, user, project, committish, path }) =>
    `https://${maybeJoin$1(auth, '@')}raw.githubusercontent.com/${user}/${project}/${maybeEncode$1(committish || 'HEAD')}/${path}`,
  gittemplate: ({ auth, domain, user, project, committish }) =>
    `git://${maybeJoin$1(auth, '@')}${domain}/${user}/${project}.git${maybeJoin$1('#', committish)}`,
  tarballtemplate: ({ domain, user, project, committish }) =>
    `https://codeload.${domain}/${user}/${project}/tar.gz/${maybeEncode$1(committish || 'HEAD')}`,
  extract: (url) => {
...
hosts$3.bitbucket = {
  ...
  edittemplate: ({ domain, user, project, committish, treepath, path, editpath }) =>
    `https://${domain}/${user}/${project}${maybeJoin$1('/', treepath, '/', maybeEncode$1(committish || 'HEAD'), '/', path, editpath)}`,
  tarballtemplate: ({ domain, user, project, committish }) =>
    `https://${domain}/${user}/${project}/get/${maybeEncode$1(committish || 'HEAD')}.tar.gz`,
  extract: (url) => {
...
hosts$3.gitlab = {
  ...
  httpstemplate: ({ auth, domain, user, project, committish }) =>
    `git+https://${maybeJoin$1(auth, '@')}${domain}/${user}/${project}.git${maybeJoin$1('#', committish)}`,
  tarballtemplate: ({ domain, user, project, committish }) =>
    `https://${domain}/${user}/${project}/repository/archive.tar.gz?ref=${maybeEncode$1(committish || 'HEAD')}`,
  extract: (url) => {
...
hosts$3.gist = {
  ...
  sshtemplate: ({ domain, project, committish }) =>
    `git@${domain}:${project}.git${maybeJoin$1('#', committish)}`,
  sshurltemplate: ({ domain, project, committish }) =>
    `git+ssh://git@${domain}/${project}.git${maybeJoin$1('#', committish)}`,
  edittemplate: ({ domain, user, project, committish, editpath }) =>
    `https://${domain}/${user}/${project}${maybeJoin$1('/', maybeEncode$1(committish))}/${editpath}`,
  browsetemplate: ({ domain, project, committish }) =>
    `https://${domain}/${project}${maybeJoin$1('/', maybeEncode$1(committish))}`,
  browsetreetemplate: ({ domain, project, committish, path, hashformat }) =>
    `https://${domain}/${project}${maybeJoin$1('/', maybeEncode$1(committish))}${maybeJoin$1('#', hashformat(path))}`,
  ...
  httpstemplate: ({ domain, project, committish }) =>
    `git+https://${domain}/${project}.git${maybeJoin$1('#', committish)}`,
  filetemplate: ({ user, project, committish, path }) =>
    `https://gist.githubusercontent.com/${user}/${project}/raw${maybeJoin$1('/', maybeEncode$1(committish))}/${path}`,
  shortcuttemplate: ({ type, project, committish }) =>
    `${type}:${project}${maybeJoin$1('#', committish)}`,
  pathtemplate: ({ project, committish }) =>
    `${project}${maybeJoin$1('#', committish)}`,
  bugstemplate: ({ domain, project }) =>
    `https://${domain}/${project}`,
  gittemplate: ({ domain, project, committish }) =>
    `git://${domain}/${project}.git${maybeJoin$1('#', committish)}`,
  tarballtemplate: ({ project, committish }) =>
    `https://codeload.github.com/gist/${project}/tar.gz/${maybeEncode$1(committish || 'HEAD')}`,
  extract: (url) => {
    let [, user, project, aux] = url.pathname.split('/', 4);
    if (aux === 'raw') {
      return
  ...
  hashformat: function (fragment) {
    return fragment && 'file-' + formatHashFragment$1(fragment)
  },
};
hosts$3.sourcehut = {
  ...
  blobpath: 'tree',
  filetemplate: ({ domain, user, project, committish, path }) =>
    `https://${domain}/${user}/${project}/blob/${maybeEncode$1(committish) || 'HEAD'}/${path}`,
  httpstemplate: ({ domain, user, project, committish }) =>
    `https://${domain}/${user}/${project}.git${maybeJoin$1('#', committish)}`,
  tarballtemplate: ({ domain, user, project, committish }) =>
    `https://${domain}/${user}/${project}/archive/${maybeEncode$1(committish) || 'HEAD'}.tar.gz`,
...
for (const [name, host] of Object.entries(hosts$3)) {
  hosts$3[name] = Object.assign({}, defaults$1, host);
}
var hosts_1$1 = hosts$3;
const url$4 = require$$0$1;
const lastIndexOfBefore$1 = (str, char, beforeChar) => {
  const startPosition = str.indexOf(beforeChar);
  return str.lastIndexOf(char, startPosition > -1 ? startPosition : Infinity)
};
const safeUrl$1 = (u) => {
  try {
    return new url$4.URL(u)
  } catch {
  }
};
const correctProtocol$1 = (arg, protocols) => {
...
const correctUrl$1 = (giturl) => {
  const firstAt = lastIndexOfBefore$1(giturl, '@', '#');
  const lastColonBeforeHash = lastIndexOfBefore$1(giturl, ':', '#');
  if (lastColonBeforeHash > firstAt) {
    giturl = giturl.slice(0, lastColonBeforeHash) + '/' + giturl.slice(lastColonBeforeHash + 1);
  }
  if (lastIndexOfBefore$1(giturl, ':', '#') === -1 && giturl.indexOf('//') === -1) {
    giturl = `git+ssh://${giturl}`;
  }
  return giturl
};
var parseUrl$5 = (giturl, protocols) => {
  const withProtocol = protocols ? correctProtocol$1(giturl, protocols) : giturl;
  return safeUrl$1(withProtocol) || safeUrl$1(correctUrl$1(withProtocol))
};
const parseUrl$4 = parseUrl$5;
const isGitHubShorthand$1 = (arg) => {
...
var fromUrl$3 = (giturl, opts, { gitHosts, protocols }) => {
  if (!giturl) {
    return
  }
  const correctedUrl = isGitHubShorthand$1(giturl) ? `github:${giturl}` : giturl;
  const parsed = parseUrl$4(correctedUrl, protocols);
  if (!parsed) {
    return
  }
...
const LRU = lruCache;
const hosts$2 = hosts_1$1;
const fromUrl$2 = fromUrl$3;
const parseUrl$3 = parseUrl$5;
const cache$2 = new LRU({ max: 1000 });
let GitHost$1 = class GitHost {
  ...
  static fromUrl (giturl, opts) {
    ...
    if (!cache$2.has(key)) {
      ...
      cache$2.set(key, hostArgs ? new GitHost(...hostArgs) : undefined);
    }
    return cache$2.get(key)
  }
  static parseUrl (url) {
...
for (const [name, host] of Object.entries(hosts$2)) {
  GitHost$1.addHost(name, host);
}
var lib$5 = GitHost$1;
...
var isCoreModule = function isCore(x, nodeVersion) {
	return has$1(data, x) && versionIncluded(nodeVersion, data[x]);
};
var extract_description$1 = extractDescription$3;
function extractDescription$3 (d) {
  if (!d) {
    return
  }
...
var isValidSemver$1 = valid_1;
var cleanSemver$1 = clean_1;
var validateLicense$1 = validateNpmPackageLicense;
var hostedGitInfo$1 = lib$5;
var isBuiltinModule$1 = isCoreModule;
var depTypes$1 = ['dependencies', 'devDependencies', 'optionalDependencies'];
var extractDescription$2 = extract_description$1;
var url$2 = require$$0$1;
var typos$1 = require$$7$1;
var isEmail$1 = str => str.includes('@') && (str.indexOf('@') < str.lastIndexOf('.'));
var fixer$3 = {
  warn: function () {},
  fixRepositoryField: function (data) {
    ...
    if (r) {
      var hosted = hostedGitInfo$1.fromUrl(r);
      if (hosted) {
  ...
  fixTypos: function (data) {
    Object.keys(typos$1.topLevel).forEach(function (d) {
      if (Object.prototype.hasOwnProperty.call(data, d)) {
        this.warn('typo', d, typos$1.topLevel[d]);
      }
    }, this);
  },
  fixScriptsField: function (data) {
    ...
    Object.keys(data.scripts).forEach(function (k) {
      ...
      } else if (typos$1.script[k] && !data.scripts[typos$1.script[k]]) {
        this.warn('typo', k, typos$1.script[k], 'scripts');
      }
    }, this);
  },
  ...
  fixDependencies: function (data, strict) {
    objectifyDeps$1(data, this.warn);
    addOptionalDepsToDeps$1(data, this.warn);
    ...
    ;['dependencies', 'devDependencies'].forEach(function (deps) {
      ...
      Object.keys(data[deps]).forEach(function (d) {
        ...
        var hosted = hostedGitInfo$1.fromUrl(data[deps][d]);
        if (hosted) {
  ...
  fixVersionField: function (data, strict) {
    ...
    if (!isValidSemver$1(data.version, loose)) {
      throw new Error('Invalid version: "' + data.version + '"')
    }
    data.version = cleanSemver$1(data.version, loose);
    return true
  },
  fixPeople: function (data) {
    modifyPeople$1(data, unParsePerson$1);
    modifyPeople$1(data, parsePerson$1);
  },
  fixNameField: function (data, options) {
    ...
    ensureValidName$1(data.name, strict, options.allowLegacyCase);
    if (isBuiltinModule$1(data.name)) {
  ...
  fixDescriptionField: function (data) {
    ...
    if (data.readme && !data.description) {
      data.description = extractDescription$2(data.readme);
    }
  ...
  fixBugsField: function (data) {
    if (!data.bugs && data.repository && data.repository.url) {
      var hosted = hostedGitInfo$1.fromUrl(data.repository.url);
      if (hosted && hosted.bugs()) {
        data.bugs = { url: hosted.bugs() };
      }
    } else if (data.bugs) {
      if (typeof data.bugs === 'string') {
        if (isEmail$1(data.bugs)) {
          data.bugs = { email: data.bugs };
        } else if (url$2.parse(data.bugs).protocol) {
          data.bugs = { url: data.bugs };
        } else {
          this.warn('nonEmailUrlBugsString');
        }
      } else {
        bugsTypos$1(data.bugs, this.warn);
        var oldBugs = data.bugs;
        data.bugs = {};
        if (oldBugs.url) {
          if (typeof (oldBugs.url) === 'string' && url$2.parse(oldBugs.url).protocol) {
        ...
        if (oldBugs.email) {
          if (typeof (oldBugs.email) === 'string' && isEmail$1(oldBugs.email)) {
  ...
  fixHomepageField: function (data) {
    ...
    if (!url$2.parse(data.homepage).protocol) {
  ...
  fixLicenseField: function (data) {
    ...
    if (!validateLicense$1(license).validForNewPackages) {
      return this.warn('invalidLicense')
    }
  },
};
function isValidScopedPackageName$1 (spec) {
  if (spec.charAt(0) !== '@') {
    return false
  }
...
function isCorrectlyEncodedName$1 (spec) {
  return !spec.match(/[/@\s+%:]/) &&
    spec === encodeURIComponent(spec)
}
function ensureValidName$1 (name, strict, allowLegacyCase) {
  if (name.charAt(0) === '.' ||
      !(isValidScopedPackageName$1(name) || isCorrectlyEncodedName$1(name)) ||
...
function modifyPeople$1 (data, fn) {
  if (data.author) {
...
function unParsePerson$1 (person) {
...
function addOptionalDepsToDeps$1 (data, warn) {
...
function depObjectify$1 (deps, type, warn) {
  if (!deps) {
    return {}
  }
...
function objectifyDeps$1 (data, warn) {
  depTypes$1.forEach(function (type) {
    if (!data[type]) {
      return
    }
    data[type] = depObjectify$1(data[type], type, warn);
  });
}
function bugsTypos$1 (bugs, warn) {
  if (!bugs) {
    return
  }
  Object.keys(bugs).forEach(function (k) {
    if (typos$1.bugs[k]) {
      warn('typo', k, typos$1.bugs[k], 'bugs');
      bugs[typos$1.bugs[k]] = bugs[k];
      delete bugs[k];
    }
  });
}
var repositories$1 = "'repositories' (plural) Not supported. Please pick one as the 'repository' field";
var missingRepository$1 = "No repository field.";
var brokenGitUrl$1 = "Probably broken git url: %s";
var nonObjectScripts$1 = "scripts must be an object";
var nonStringScript$1 = "script values must be string commands";
var nonArrayFiles$1 = "Invalid 'files' member";
var invalidFilename$1 = "Invalid filename in 'files' list: %s";
var nonArrayBundleDependencies$1 = "Invalid 'bundleDependencies' list. Must be array of package names";
var nonStringBundleDependency$1 = "Invalid bundleDependencies member: %s";
var nonDependencyBundleDependency$1 = "Non-dependency in bundleDependencies: %s";
var nonObjectDependencies$1 = "%s field must be an object";
var nonStringDependency$1 = "Invalid dependency: %s %s";
var deprecatedArrayDependencies$1 = "specifying %s as array is deprecated";
var deprecatedModules$1 = "modules field is deprecated";
var nonArrayKeywords$1 = "keywords should be an array of strings";
var nonStringKeyword$1 = "keywords should be an array of strings";
var conflictingName$1 = "%s is also the name of a node core module.";
var nonStringDescription$1 = "'description' field should be a string";
var missingDescription$1 = "No description";
var missingReadme$1 = "No README data";
var missingLicense$1 = "No license field.";
var nonEmailUrlBugsString$1 = "Bug string field must be url, email, or {email,url}";
var nonUrlBugsUrlField$1 = "bugs.url field must be a string url. Deleted.";
var nonEmailBugsEmailField$1 = "bugs.email field must be a string email. Deleted.";
var emptyNormalizedBugs$1 = "Normalized value of bugs field is an empty object. Deleted.";
var nonUrlHomepage$1 = "homepage field must be a string url. Deleted.";
var invalidLicense$1 = "license should be a valid SPDX license expression";
var typo$1 = "%s should probably be %s.";
const require$$1$1 = {
	repositories: repositories$1,
	missingRepository: missingRepository$1,
	brokenGitUrl: brokenGitUrl$1,
	nonObjectScripts: nonObjectScripts$1,
	nonStringScript: nonStringScript$1,
	nonArrayFiles: nonArrayFiles$1,
	invalidFilename: invalidFilename$1,
	nonArrayBundleDependencies: nonArrayBundleDependencies$1,
	nonStringBundleDependency: nonStringBundleDependency$1,
	nonDependencyBundleDependency: nonDependencyBundleDependency$1,
	nonObjectDependencies: nonObjectDependencies$1,
	nonStringDependency: nonStringDependency$1,
	deprecatedArrayDependencies: deprecatedArrayDependencies$1,
	deprecatedModules: deprecatedModules$1,
	nonArrayKeywords: nonArrayKeywords$1,
	nonStringKeyword: nonStringKeyword$1,
	conflictingName: conflictingName$1,
	nonStringDescription: nonStringDescription$1,
	missingDescription: missingDescription$1,
	missingReadme: missingReadme$1,
	missingLicense: missingLicense$1,
	nonEmailUrlBugsString: nonEmailUrlBugsString$1,
	nonUrlBugsUrlField: nonUrlBugsUrlField$1,
	nonEmailBugsEmailField: nonEmailBugsEmailField$1,
	emptyNormalizedBugs: emptyNormalizedBugs$1,
	nonUrlHomepage: nonUrlHomepage$1,
	invalidLicense: invalidLicense$1,
	typo: typo$1
};
var util$2 = require$$0;
var messages$1 = require$$1$1;
var make_warning$1 = function () {
  var args = Array.prototype.slice.call(arguments, 0);
  var warningName = args.shift();
  if (warningName === 'typo') {
    return makeTypoWarning$1.apply(null, args)
  } else {
...
function makeTypoWarning$1 (providedName, probableName, field) {
  if (field) {
    providedName = field + "['" + providedName + "']";
    probableName = field + "['" + probableName + "']";
  }
  return util$2.format(messages$1.typo, providedName, probableName)
}
var normalize_1$1 = normalize$1;
var fixer$2 = fixer$3;
normalize$1.fixer = fixer$2;
var makeWarning$1 = make_warning$1;
var fieldsToFix$1 = ['name', 'version', 'description', 'repository', 'modules', 'scripts',
  'files', 'bin', 'man', 'bugs', 'keywords', 'readme', 'homepage', 'license'];
var otherThingsToFix$1 = ['dependencies', 'people', 'typos'];
var thingsToFix$1 = fieldsToFix$1.map(function (fieldName) {
  return ucFirst$1(fieldName) + 'Field'
});
thingsToFix$1 = thingsToFix$1.concat(otherThingsToFix$1);
function normalize$1 (data, warn, strict) {
  ...
  fixer$2.warn = function () {
    warn(makeWarning$1.apply(null, arguments));
  };
  thingsToFix$1.forEach(function (thingName) {
    fixer$2['fix' + ucFirst$1(thingName)](data, strict);
  });
...
const normalizePackageData$1 = getDefaultExportFromCjs(normalize_1$1);
const typeMappings = {
...
const errorEx$1 = getDefaultExportFromCjs(errorEx_1);
const hexify = char => {
...
const fallback = getDefaultExportFromCjs(lib$4);
var lib$3 = {};
var lib$2 = {};
var jsTokens = {};
...
var chalkExports$1 = chalk$3.exports;
...
var _chalk2$1 = chalkExports$1;
const chalk$2 = _chalk2$1;
...
function shouldHighlight(options) {
  return !!chalk$2.supportsColor || options.forceColor;
}
let chalkWithForcedColor$1 = undefined;
function getChalk$1(forceColor) {
  if (forceColor) {
    var _chalkWithForcedColor;
    (_chalkWithForcedColor = chalkWithForcedColor$1) != null ? _chalkWithForcedColor : chalkWithForcedColor$1 = new chalk$2.constructor({
      enabled: true,
      level: 1
    });
...
{
  {
    lib$2.getChalk = options => getChalk$1(options.forceColor);
  }
}
...
var codeFrameColumns_1 = lib$3.codeFrameColumns = codeFrameColumns;
lib$3.default = _default;
var _highlight = lib$2;
var _chalk2 = chalkExports;
const chalk = _chalk2;
...
const JSONError = errorEx$1('JSONError', {
	fileName: errorEx$1.append('in %s'),
	codeFrame: errorEx$1.append('\n\n%s\n'),
});
function parseJson(string, reviver, filename) {
	...
	try {
		...
		if (indexMatch && indexMatch.length > 0) {
			...
			const codeFrame = codeFrameColumns_1(
				string,
...
export { readPackageUpSync as a, constructParserOptions as b, camelcaseKeys as c, decamelizeKeys as d, decamelize as e, hardRejection$1 as h, normalizePackageData$1 as n, redent as r, trimNewlines as t, yargsParser as y };
