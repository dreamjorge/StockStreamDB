    ...
    def get_stock_data(self, ticker: str, period: str = "1mo", retries: int = 3):
        ...
        while attempt < retries:
            try:
                ...
                print(
                    f"Connection error while attempting to fetch data for {ticker}. Retrying ({attempt + 1}/{retries})..."
                )
                print(e)
                attempt += 1
        ...
        print(
            f"Connection error: Failed to retrieve data for {ticker} after {retries} attempts."
        )
        return None
