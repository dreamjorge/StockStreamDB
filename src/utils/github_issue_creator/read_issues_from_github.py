...
def export_issues_to_yaml(output_file):
    ...
    for issue in issues:
        issue_data = {
            "prefix": (
                "[%s]" % issue.labels[0].name if issue.labels else ""
            ),  # Assuming first label as prefix
            "title": issue.title,
            "description": issue.body or "",
            "labels": [label.name for label in issue.labels],
            "milestone": issue.milestone.title if issue.milestone else None,
            "milestone_description": (
                issue.milestone.description if issue.milestone else ""
            ),
            "milestone_due_on": (
                issue.milestone.due_on.strftime("%Y-%m-%d")
