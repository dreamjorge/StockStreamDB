...
)  # Set a default owner if not in .env
REPO_NAME = os.getenv(
    "REPO_NAME", "your_repo_name"
)  # Set a default repo if not in .env
...
class GitHubManager:
    ...
    def create_label_if_not_exists(self, label_name, color="ffffff", description=""):
        ...
        try:
            ...
            if "already_exists" in str(e):
                logging.info(
                    f"Label '{label_name}' already exists according to the API.\
                    Skipping."
                )
            else:
                logging.error(
                    f"Failed to create label '{label_name}': {traceback.format_exc()}"
                )
            return None

    def get_cached_milestones(self):
        if self.cached_milestones is None:
            self.cached_milestones = {
    ...
    def create_milestone_if_not_exists(
        self, milestone_title, description="Milestone created via script", due_on=None
    ):
        milestones = self.get_cached_milestones()
        if milestone_title in milestones:
            logging.info(
                f"Milestone '{milestone_title}' already exists. Skipping creation."
            )
    ...
    def get_cached_issues(self):
        if self.cached_issues is None:
            self.cached_issues = {
                issue.title: issue for issue in self.repo.get_issues(state="open")
            }
...
class IssueManager:
    ...
    def update_issue_if_needed(self, issue, labels, milestone_title, label_colors):
        ...
        valid_labels = [
            label.name
            for label in self.github_manager.get_cached_labels().values()
        ...
        if milestone_title:
            milestone = self.github_manager.create_milestone_if_not_exists(
                milestone_title
            )
            if issue.milestone is None or (
                issue.milestone and issue.milestone.title != milestone_title
            ):
    ...
    def create_or_update_issue(
        ...
        if title in issues:
            issue = issues[title]
            logging.info(
                f"Issue with title '{title}' already exists. Checking for updates."
            )
        ...
        if labels:
            ...
            normalized_existing_labels = {
                label.name.lower(): label for label in existing_labels
            }
            ...
            for label in labels:
                ...
                if normalized_label in normalized_existing_labels:
                    ...
                    self.github_manager.create_label(
                        label, label_colors.get(label, "FFFFFF")
                    )

        valid_labels = [
            label
            for label in self.github_manager.get_cached_labels().values()
            if label.name.lower() in [label.lower() for label in labels]
        ]
    ...
    def check_rate_limit(self):
        ...
        if rate_limit.remaining == 0:
            ...
            logging.error(
                f"GitHub API rate limit exceeded. Limit will reset at {reset_time}."
            )
            return False
        logging.info(
            f"GitHub API rate limit: {rate_limit.remaining}/{rate_limit.limit}"
        )
        return True
